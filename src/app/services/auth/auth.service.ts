...
export class AuthService {
  ...
  login(credentials:LoginRequest): Observable<any> {
    return this.http.post<any>(`${environment.API_URL_AUTH}/login`, credentials, httpOptions).pipe(
      tap((userData) => {
  ...
  register(credentials:RegisterRequest): Observable<any> {
    return this.http.post<any>(`${environment.API_URL_AUTH}/register`, credentials, httpOptions).pipe(
      tap((userData) => {
        sessionStorage.setItem("access_token", userData.token);
        this.userToken.next(userData.token);
        this.isUserLogged.next(true);
